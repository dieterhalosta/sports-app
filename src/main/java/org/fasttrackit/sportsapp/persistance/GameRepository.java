...
public interface GameRepository extends JpaRepository<Game, Long> {
    ...
    @Query("SELECT user.firstName, user.lastName, event.description, event.date, event.imageUrl, event.location, event.name, event.participants FROM User user INNER JOIN Game game ON  =user.id INNER JOIN Event event ON game_user.game_id=event.id WHERE (:activeUserId IS null or user.id LIKE %:activeUserId%)")
