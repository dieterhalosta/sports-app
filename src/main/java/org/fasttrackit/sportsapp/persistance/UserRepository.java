...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT user.firstName, user.lastName, event.description, event.date, event.imageUrl, event.location, event.name, event.participants FROM User user JOIN Game game ON user.id=game.mainUser.id JOIN Event event ON game.id=event.id WHERE (:activeUserId IS null or user.id = :activeUserId)")
